...
//Constructor.
void UnitAINode:: startUp(std::vector<bool*>& preconditions, std::vector<UnitAINode*>& nodes)
{
...
std::vector<UnitAINode*> UnitAINode::getBestPath()
{
	std::vector<UnitAINode*> pathToReturn;
	int cost = 0;
	...
	for (int i = 0; i < m_preconditionNodes.size(); i++)
	{
		for (int j = 0; j < m_preconditionNodes.size(); j++)
		{
			if (m_preconditionNodes[j]->checkPreconditions())
			{
				std::vector<UnitAINode*> temp = m_preconditionNodes[j]->getBestPath();
				if (temp.size() > 0)
				{
...
bool UnitAINode::checkPreconditions()
{
...
bool UnitAINode::checkNodePreconditions()
{
